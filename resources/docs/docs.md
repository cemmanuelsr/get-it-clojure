...
The `home-routes` handler in the `get-it.routes.home` namespace
...
The `home-page` function will in turn call the `get-it.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `get-it.handler` namespace:
...
Request middleware functions are located under the `get-it.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `get-it.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `get-it.db.core` in some other namespace.
